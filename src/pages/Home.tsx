...
export const Home = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-[#f8fafc] font-sans selection:bg-clinical selection:text-white relative text-slate-800 overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full px-8 py-6 z-50 flex justify-between items-center bg-white/70 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all duration-300">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-clinical rounded-lg flex items-center justify-center text-white font-serif italic shadow-md">
             V
          </div>
          <div className="flex flex-col">
             <span className="text-clinical font-bold text-lg leading-none tracking-tight">VoluTB</span>
        ...
        <div className="flex gap-8 items-center text-xs uppercase tracking-wider font-bold text-slate-600">
          <a href="#how-it-works" className="cursor-pointer hover:text-clinical transition-colors">Proses</a>
          <a href="#about" className="cursor-pointer hover:text-clinical transition-colors">Teknologi</a>
        </div>
      </nav>
      ...
      <section className="relative w-full min-h-screen flex items-center justify-center pt-20 px-8 z-10">
        <div className="relative z-10 w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial="hidden" animate="visible" variants={containerVariants} className="flex flex-col items-start text-left max-w-2xl">
            <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-clinical/30 bg-clinical/10 text-clinical text-[10px] uppercase font-bold tracking-widest mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-clinical animate-pulse"></span>
            ...
            <motion.h1 variants={itemVariants} className="text-5xl md:text-7xl font-bold mb-6 tracking-tight leading-[1.1] text-slate-800">
              Transformasi <br/><span className="text-clinical">Onkologi Spasial.</span>
            </motion.h1>
            ...
            <motion.div variants={itemVariants} className="flex gap-4">
              <button 
                ...
                className="bg-clinical text-white px-8 py-4 rounded-xl font-bold hover:bg-clinical-dark transition-all shadow-md shadow-clinical/20 flex items-center gap-2"
              >
                Akses Dasbor Medis
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, ease: customEase, delay: 0.2 }} className="relative w-full h-[600px] flex items-center justify-center">
             {/* Clinical Hero Illustration Placeholder */}
             <div className="w-full h-full bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.05)] border border-slate-100 p-4 relative overflow-hidden flex flex-col">
                ...
                <div className="flex-1 bg-slate-50 rounded-xl relative overflow-hidden flex items-center justify-center border border-slate-100">
                   ...
                   <div className="absolute inset-0 grid grid-cols-2 gap-4 p-8">
                      ...
                      <div className="bg-clinical/5 rounded-lg border border-clinical/20 shadow-sm p-4 flex flex-col justify-center items-center relative">
                         <div className="absolute top-4 right-4 text-clinical"><Activity size={20} /></div>
                         <div className="w-48 h-48 bg-white rounded-full mb-4 flex items-center justify-center shadow-lg border border-clinical/20 relative">
                            <div className="w-32 h-32 rounded-full border-2 border-dashed border-clinical/50 flex items-center justify-center">
                               <div className="w-16 h-16 bg-clinical/20 rounded-full animate-ping"></div>
                               <div className="absolute w-2 h-2 bg-clinical rounded-full"></div>
                            </div>
                         </div>
                         <span className="text-clinical font-bold text-xs uppercase tracking-widest">Pseudo-CT 3D</span>
                      </div>
                   </div>
                </div>
             </div>
      ...
      <section id="how-it-works" className="relative w-full py-32 px-8 z-10 bg-white border-y border-slate-200">
        <div className="relative z-10 w-full max-w-6xl mx-auto flex flex-col items-center">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={containerVariants} className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
            ...
            {/* Step 1 */}
            <motion.div variants={itemVariants} className="bg-[#f8fafc] border border-slate-200 p-10 rounded-[2rem] shadow-sm hover:shadow-md hover:border-clinical/30 transition-all">
              <div className="flex justify-between items-start mb-8">
                <div className="w-14 h-14 bg-white border border-slate-200 rounded-2xl flex items-center justify-center shadow-sm">
                  <ScanLine size={24} className="text-clinical" />
                </div>
            ...
            <motion.div variants={itemVariants} className="bg-[#f8fafc] border border-slate-200 p-10 rounded-[2rem] shadow-sm hover:shadow-md hover:border-clinical/30 transition-all">
              <div className="flex justify-between items-start mb-8">
                <div className="w-14 h-14 bg-white border border-slate-200 rounded-2xl flex items-center justify-center shadow-sm">
                  <Layers size={24} className="text-clinical" />
                </div>
            ...
            <motion.div variants={itemVariants} className="bg-[#f8fafc] border border-slate-200 p-10 rounded-[2rem] shadow-sm hover:shadow-md hover:border-clinical/30 transition-all">
              <div className="flex justify-between items-start mb-8">
                <div className="w-14 h-14 bg-white border border-slate-200 rounded-2xl flex items-center justify-center shadow-sm">
                  <BrainCircuit size={24} className="text-clinical" />
                </div>
      ...
      <section id="gateways" className="relative w-full min-h-screen flex items-center justify-center py-32 px-8 z-10 bg-[#f0f4f8]">
        <div className="relative z-10 w-full max-w-5xl flex flex-col items-center">
          ...
          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: customEase }} viewport={{ once: true }} className="text-4xl md:text-5xl font-bold mb-4 text-center tracking-tight text-slate-800">
            Akses <span className="text-clinical">Portal</span>
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: customEase, delay: 0.1 }} viewport={{ once: true }} className="text-slate-500 text-center text-lg max-w-2xl mb-20 font-medium">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={containerVariants} className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
            ...
            <motion.div 
              ...
              whileHover={{ y: -5 }}
              onClick={() => navigate('/klinisi/login')} 
              className="group cursor-pointer bg-white border border-slate-200 p-10 rounded-[2.5rem] shadow-sm hover:shadow-xl hover:border-clinical/30 transition-all duration-300 relative overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-clinical/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              
              <div className="flex justify-between items-start mb-16 relative z-10">
                <div className="w-16 h-16 rounded-[1.5rem] bg-clinical/10 border border-clinical/20 flex items-center justify-center group-hover:bg-clinical transition-all">
                  <Activity size={28} className="text-clinical group-hover:text-white transition-colors" />
                </div>
                <span className="text-[10px] uppercase tracking-widest font-bold text-slate-400 group-hover:text-clinical transition-colors">[01] GATEWAY</span>
              </div>
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center justify-between border-t border-slate-100 pt-6">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-clinical transition-colors">Authenticate</span>
                  <div className="w-10 h-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center group-hover:bg-clinical group-hover:border-clinical group-hover:text-white transition-all text-slate-600">
                    <ArrowUpRight size={18} />
                  </div>
                </div>
              </div>
